...
msg2: .asciiz "De estos 3 números el número mayor es: " 	# Declara una cadena de caracteres
...
main:						# Sección de código principal
    ...
    move $t2, $v0				# Mueve el entero ingresado al registro $t1
    
    li $v0, 4					# Código de servicio 4 para imprimir una cadena
    la $a0, msg2					# Carga la dirección de la cadena en $a0
    syscall					# Llama al sistema para imprimir
    
    bge $t0, $t1, comp1				# Si $t0 es mayor o igual a $t1 va a la sección comp1, de lo contrario continúa
    move $t3, $t1				# Mueve al registro $t3 el valor de $t1, ya que este es mayor que $t0
    j comp1f					# Salta al final del código de la comparación entre $t0 y $t1

comp1:						# Aquí llega si $t0 es mayor o igual a $t1
    move $t3, $t0				# Mueve a $t3 el valor de $t0, ya que es mayor o igual a $t1
    j comp1f					# Salta al final del código de la comparación entre $t0 y $t1
    
comp1f:						# Aquí ya terminó la comparación entre $t0 y $t1
    
    bge $t2, $t3, comp2				# Comparamos $t2 con el mayor entre $t0 y $t1 que está en $t3
    j comp2f					# Aquí $t2 es menor que $t3, entonces pasamos al final de esta comparación
    
comp2:						# Aquí llegamos si $t2 es mayor o igual a $t3
    move $t3, $t2				# Movemos a $t3 el valor de $t2 por ser mayor
    j comp2f					# Pasamos al final de la segunda comparación
    
comp2f:						# Hemos comparado $t0, $t1 y $t2 y el mayor está en $t3
